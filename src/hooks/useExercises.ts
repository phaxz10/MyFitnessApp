...
export function useExercises() {
  ...
  const addExercise = useCallback(async (exercise: Omit<Exercise, 'id' | 'created_at'>): Promise<number> => {
    setLoading(true);
    setError(null);
    try {
      const db = await getDB();
      const result = await db.query(
        `INSERT INTO exercises (name, description, muscle_groups, equipment, is_ai_generated)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id`,
        [exercise.name, exercise.description, exercise.muscle_groups, exercise.equipment, exercise.is_ai_generated]
      );
  ...
  const updateExercise = useCallback(async (id: number, data: Partial<Exercise>) => {
    setLoading(true);
    setError(null);
    try {
      const db = await getDB();
