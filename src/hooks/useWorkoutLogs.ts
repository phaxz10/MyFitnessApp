...
export function useWorkoutLogs() {
  ...
  const [activeWorkoutSets, setActiveWorkoutSets] = useState<WorkoutSetWithExercise[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const fetchLogs = useCallback(async (limit = 20): Promise<WorkoutLogWithSets[]> => {
    setLoading(true);
    setError(null);
    try {
      ...
      const workoutLogs = result.rows as (WorkoutLog & { session_name?: string; program_name?: string })[];
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch workout logs');
      return [];
    } finally {
      setLoading(false);
    }
  ...
  const getLogsByDateRange = useCallback(async (startDate: string, endDate: string): Promise<WorkoutLogWithSets[]> => {
    try {
      const db = await getDB();
      const result = await db.query(
        `SELECT wl.*, ps.name as session_name, wp.name as program_name
  ...
  const endWorkout = useCallback(async (workoutId: number, notes?: string): Promise<void> => {
    setLoading(true);
    setError(null);
    try {
      const db = await getDB();
  ...
  const cancelWorkout = useCallback(async (workoutId: number): Promise<void> => {
    setLoading(true);
    setError(null);
    try {
      const db = await getDB();
  ...
  const addSet = useCallback(async (
    ...
    try {
      ...
      const exerciseName = (exerciseResult.rows as { name: string }[])[0]?.name || '';
  ...
  const getExerciseHistory = useCallback(async (
    ...
    try {
      ...
      if ((exerciseResult.rows as { name: string }[]).length === 0) return null;
      const exerciseName = (exerciseResult.rows as { name: string }[])[0].name;
  ...
  const loadActiveWorkoutSets = useCallback(async (workoutLogId: number): Promise<void> => {
    try {
      const db = await getDB();
      const result = await db.query(
        `SELECT ws.*, e.name as exercise_name
