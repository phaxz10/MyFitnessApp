  ...
  return (
    <div className="p-4 pb-20 space-y-4">
      ...
      <div className="text-center mb-6">
        ...
        <h2 className="text-xl font-semibold text-white">{formatDisplayDate(today)}</h2>
      </div>
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/calories" className="text-blue-400 text-sm hover:underline">
              View Details
            </Link>
          </div>
          ...
          <div className="flex items-center justify-center mb-6">
            <div className="relative w-40 h-40">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-bold text-white">{formatCalories(caloriesRemaining)}</span>
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-2xl font-semibold text-white">{formatCalories(caloriesConsumed)}</p>
              <p className="text-slate-400 text-xs">Consumed</p>
            </div>
            <div>
              <p className="text-2xl font-semibold text-white">{formatCalories(calorieTarget)}</p>
              <p className="text-slate-400 text-xs">Target</p>
            </div>
            <div>
              <p className={`text-2xl font-semibold ${caloriesRemaining < 0 ? 'text-red-400' : 'text-green-400'}`}>
                {formatCalories(Math.abs(caloriesRemaining))}
              </p>
              <p className="text-slate-400 text-xs">{caloriesRemaining < 0 ? 'Over' : 'Left'}</p>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-white">{summary.total_protein_g.toFixed(0)}g / {profile?.protein_target_g || 0}g</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-slate-400">Carbs</span>
              <span className="text-white">{summary.total_carbs_g.toFixed(0)}g / {profile?.carbs_target_g || 0}g</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-slate-400">Fat</span>
              <span className="text-white">{summary.total_fat_g.toFixed(0)}g / {profile?.fat_target_g || 0}g</span>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Link to="/scanner">
          <Card className={`transition-colors ${isOnline ? 'hover:bg-slate-700' : 'opacity-50'}`}>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs">{isOnline ? 'AI analysis' : 'Requires internet'}</p>
              </div>
            </CardContent>
          </Card>
        </Link>
          ...
          <Card className="hover:bg-slate-700 transition-colors">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs">{latestWeight ? `${latestWeight} kg` : 'Track progress'}</p>
              </div>
            </CardContent>
          </Card>
        </Link>
      ...
      <Card>
        <CardContent className="p-4">
          ...
          {Object.entries(summary.meals).map(([mealType, mealEntries]) => {
            ...
            return (
              <div key={mealType} className="flex justify-between items-center py-2 border-b border-slate-700 last:border-0">
                <div className="flex items-center gap-3">
                  <Utensils size={16} className="text-slate-400" />
                  <span className="text-white capitalize">{mealType}</span>
                  {mealEntries.length > 0 && (
                    <span className="text-slate-500 text-sm">({mealEntries.length} items)</span>
                  )}
                </div>
