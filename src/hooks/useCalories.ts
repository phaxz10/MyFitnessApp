...
export function useCalories() {
  ...
  const fetchEntriesByDateRange = useCallback(async (startDate: string, endDate: string) => {
    setLoading(true);
    setError(null);
    try {
      const db = await getDB();
  ...
  const addEntry = useCallback(async (entry: Omit<FoodEntry, 'id' | 'created_at' | 'updated_at'>) => {
    setLoading(true);
    setError(null);
    try {
      const db = await getDB();
      await db.query(
        ...
        [
          entry.date, entry.meal_type, entry.food_description, entry.portion_grams,
          entry.calories, entry.protein_g, entry.carbs_g, entry.fat_g, entry.is_ai_generated
        ]
      );
  ...
  const updateEntry = useCallback(async (id: number, data: Partial<FoodEntry>) => {
    setLoading(true);
    setError(null);
    try {
      const db = await getDB();
  ...
  const deleteEntry = useCallback(async (id: number, date: string) => {
    setLoading(true);
    setError(null);
    try {
      const db = await getDB();
  ...
  const getDailySummary = useCallback((date: string, entriesList?: FoodEntry[]): DailyCalorieSummary => {
    const dayEntries = (entriesList || entries).filter(e => e.date === date);
